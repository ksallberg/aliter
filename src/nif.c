...
#define calc_index(x, y) (((x) + (y) * MAX_WALKPATH) & \
                          (MAX_WALKPATH * MAX_WALKPATH - 1))
...
static void push_heap_path(int *heap,struct tmp_path *tp,int index) {
  ...
  for (i = (h - 1) / 2; h > 0 && tp[index].cost < tp[heap[i + 1]].cost;
       i = (h - 1) / 2) {
...
static int pop_heap_path(int *heap,struct tmp_path *tp) {
  ...
  for (i = (h - 1) / 2; h > 0 && tp[heap[i + 1]].cost > tp[last].cost;
       i = (h - 1) / 2 ) {
    heap[h+1] = heap[i+1], h = i;
  }
...
static int add_path(int *heap, struct tmp_path *tp, int x,
                    int y, int dist, int before, int cost) {
  int i;
...
ERL_NIF_TERM finish(ErlNifEnv *env, ERL_NIF_TERM *path,
                    ERL_NIF_TERM *step, int length) {
  int i;
...
ERL_NIF_TERM pathfind(ErlNifEnv *env,
  ...
  while (1) {
    ...
    if((f & (2+1)) == (2+1) && !at(map, x - 1, y + 1))
      e += add_path(heap, tp, x - 1, y + 1, dist + 4,
                    rp, cost + dc[1] + dc[0] - 6); // (x-1, y+1)

    if((f & (2+4)) == (2+4) && !at(map, x - 1, y - 1))
      e += add_path(heap, tp, x - 1, y - 1, dist + 4,
                    rp, cost + dc[1] + dc[2] - 6); // (x-1, y-1)

    if((f & (8+4)) == (8+4) && !at(map, x + 1, y - 1))
      e += add_path(heap, tp, x + 1, y - 1, dist + 4,
                    rp, cost + dc[3] + dc[2] - 6); // (x+1, y-1)

    if((f & (8+1)) == (8+1) && !at(map, x + 1, y + 1))
      e += add_path(heap, tp, x + 1, y + 1, dist + 4,
                    rp, cost + dc[3] + dc[0] - 6); // (x+1, y+1)
